...
function App() {
    ...
    const onRegister = (props: RestrationDetails) => {
        ...
        //query database + pass result to
        client
            .mutate({ mutation: mutation(props)?.mutation })
            .then((result) => {
                if (result.data.createUser.userID > 0) {
                    toast({
                        title: "Account Created",
                        description: "Your account has been created, please check your email for the email validation token.",
                        status: "success",
                        duration: 2000,
                        isClosable: true,
                        position: "top",
    ...
    return (
        <>
            <Header toggleColorMode={toggleColorMode} toggleLogIn={onShowLogin} toggleLogout={onShowLogout} userTitle={userTitle} authenticated={authenticated}></Header>
            <Login disabled={disableInput} visible={loginVisible} onOpenRegister={onShowRegister} onLogin={onLogin} onClose={onLogInClose}></Login>
            <Logout visible={logoutVisible} onLogout={onLogout} onClose={onLogoutClose}></Logout>
